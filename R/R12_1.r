...
evals %>% transform(gender_img = gender %>% { ifelse(. == "male", "m", "j") })
evals %>%
  aggregate(. ~ gender, data = ., FUN = length) %>%
  .[c("gender", "score")]
evals %>%
  aggregate(. ~ gender + language, data = ., FUN = length) %>%
  .[c("gender", "language", "score")]
describe(evals)
...
mtcars %>%
  .[c("cyl", "mpg", "disp")] %>%
  group_by(cyl) %>%
  summarise(mean_disp = mean(disp), min_mpg = min(mpg), max_mpg = max(mpg))
mtcars %>%
  .[c("cyl", "mpg", "disp")] %>%
  group_by(cyl) %>%
  summarise(mean_mpg = mean(mpg) * 1.6093)
mtcars %>%
  .[c("cyl", "mpg", "hp")] %>%
  mutate(mpg = mpg * 1.6093) %>%
  ggplot(., aes(mpg, hp)) +
  geom_point() +
  geom_smooth()
